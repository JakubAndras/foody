...
[CONTENT TO BE FILLED: Popis GetX jako zvoleného řešení pro správu stavu. Dva základní vzory: GetxService (dlouhodobé, app-scoped služby) a GetxController (UI stav a business logika). Přístupový vzor: static FooService get to => Get.find(). Registrace závislostí v locator.dart pomocí Get.put() (permanentní) a Get.lazyPut() (lazy inicializace). Reaktivní stav pomocí Rx typů (RxList, RxBool, Obx). Celkem 29 služeb a 13 kontrolerů. BaseController jako společný předek kontrolerů.]

#### 4.3.3 Navigace a Struktura Obrazovek

[CONTENT TO BE FILLED: Přímá navigace pomocí Get.to(() => const SomeScreen()) bez pojmenovaných rout. Hlavní shell: MainScreen s třemi záložkami (Dashboard, Progress, Profile) a plovoucím akčním tlačítkem (FAB) pro rychlé akce (QuickActionSheet). Celkem přibližně 60 obrazovek organizovaných do tematických skupin (onboarding, scanning, meals, exercises, profile). Flow obrazovek pro hlavní scénáře (scan → preview → result → edit).]

### 4.4 Datový Model

[CONTENT TO BE FILLED: Scope paragraph. Uvést, že sekce popisuje strukturu lokální databáze, vztahy mezi entitami, systém šablon a repozitářovou vrstvu, která transformuje databázové entity na doménové modely.]

#### 4.4.1 Schéma Databáze a Vztahy Mezi Entitami

[CONTENT TO BE FILLED: Popis 9 entit (DayRecord, Meal, Ingredient, Exercise, WeightEntry, MealTemplate, MealTemplateIngredient, IngredientTemplate, ExerciseTemplate). Normalizované FK schéma: Meal.dayRecordId → DayRecord, Ingredient.mealId → Meal, Exercise.dayRecordId → DayRecord, vše s CASCADE delete. ER diagram jako Obr. 4.2. Tabulka s přehledem entit a jejich klíčových atributů jako Tab. 4.3. Vysvětlení rozhodnutí pro normalizované schéma oproti vnořeným seznamům (integrita dat, kaskádové mazání, dotazovatelnost).]

[FIGURE PLACEHOLDER: ER diagram databázového schématu aplikace Foody — entity DayRecord, Meal, Ingredient, Exercise, WeightEntry a šablonové entity s vyznačenými cizími klíči a kardinalitami]

**Obr. 4.2** ER diagram databázového schématu aplikace — transakční entity a šablonové entity s vyznačenými vztahy

**Tab. 4.3** Přehled databázových entit aplikace Foody — název entity, klíčové atributy a typ vztahu k nadřazené entitě

[CONTENT TO BE FILLED: Markdown tabulka s entitami, klíčovými atributy a FK vztahy]

#### 4.4.2 Systém Šablon pro Opakovaně Používané Položky

[CONTENT TO BE FILLED: Popis tří šablonových entit: MealTemplate, IngredientTemplate, ExerciseTemplate. Účel: deduplikace opakovaně zapisovaných položek, zrychlení zápisu (FR-17, FR-18, FR-19). Mechanismus normalizace názvů (normalizedName: lowercase, trimmed, unique index). Sledování frekvence použití (usageCount, lastUsedAt) pro řazení v nabídce. Vazba MealTemplate → MealTemplateIngredient pro uchování skladby šablony. Automatická tvorba šablon při uložení nového jídla/ingredience/cvičení (upsert logika).]

#### 4.4.3 Repozitářová Vrstva a Doménové Modely

[CONTENT TO BE FILLED: Popis 5 repozitářů (DayRecordRepository, WeightEntryRepository, MealTemplateRepository, IngredientTemplateRepository, ExerciseTemplateRepository). DayRecordRepository jako hlavní agregátor: kombinuje DayRecordDao, MealDao, IngredientDao, ExerciseDao do doménových objektů (DayRecord + Meal[] + Ingredient[] + Exercise[]). Transformace entity → doménový model. Reaktivní streamy (watchDayRecords, watchEntries) pro real-time aktualizace UI přes GetX. Template repositories: in-memory cache pomocí RxList, automatický refresh. Normalizace dat na úrovni repozitáře (normalizace data na midnight UTC).]
