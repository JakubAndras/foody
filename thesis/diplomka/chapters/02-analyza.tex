...
\begin{table}[ht]
	\centering
	\caption{Hodnoty BMI a jejich interpretace podle Světové zdravotnické organizace.}
	\label{tab:bmi}
	\begin{tabular}{ll}
		\toprule
...
Dostatečný příjem bílkovin je klíčový pro udržení svalové hmoty, zejména při redukci tělesné hmotnosti, a může zvyšovat pocit sytosti~\cite{venn2020macronutrients}.
...
Z~praktického hlediska je důležité nejen celkové množství energie, ale také kvalita jednotlivých makroživin: upřednostnění komplexních sacharidů a vyššího příjmu vlákniny před jednoduchými cukry, vyšší zastoupení nenasycených mastných kyselin v~porovnání s~nasycenými a dostatečný příjem bílkovin pro udržení svalové hmoty, zejména při redukci hmotnosti.
...
Odborná literatura zatím neobsahuje nezávislou kvantitativní studii přesnosti této konkrétní funkce, což odpovídá obecnějšímu trendu, kdy detailní vědecká validace komerčních funkcí postavených na umělé inteligenci obvykle následuje s~časovým odstupem za jejich praktickým nasazením~\cite{samad2022smartphoneApps,li2024evaluating,aburub2025foodScanning}.
...
MyFitnessPal a Kalorické Tabulky představují dvě reprezentativní vyústění trendu popsaného v~úvodu této sekce.
Na jedné straně demonstrují vyspělost trhu (rozsáhlé databáze, škálu vstupních metod, robustní ekosystém a milionové uživatelské základny), na straně druhé potvrzují, že základní model zůstává obdobný jako v~počátcích mobilních dietních aplikací~\cite{myFitnessPal2025,dine4fit2025,houska2024kalorickeTabulky}.
