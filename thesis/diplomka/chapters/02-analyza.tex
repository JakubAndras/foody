...
V~reakci na tato omezení se prosazuje využití umělé inteligence a počítačového vidění, avšak ve vzorku 80 podrobně analyzovaných aplikací byla identifikována pouze jediná schopná z~fotografie automaticky určit druh jídla i velikost porce~\cite{samad2022smartphoneApps}.
Plně automatické rozpoznávání talíře typicky vyžaduje potvrzení uživatele, zatímco skenování čárových kódů se stalo běžným standardem~\cite{aburub2025foodScanning}.
Nezávislá akademická validace přesnosti komerčních AI funkcí v~recenzované literatuře přitom nadále chybí.

Pro detailní analýzu byly vybrány čtyři aplikace pokrývající dvě osy trhu: tradiční přístup s~AI jako asistentem zápisu oproti přístupu stavícímu primárně na AI rozpoznávání, a~globální oproti lokální působnost.
Tradiční skupinu zastupují globální MyFitnessPal a československé Kalorické Tabulky (Dine4Fit) s~milionovou tuzemskou základnou.
Druhá skupina, navazující na třetí vlnu multimodálních velkých jazykových modelů uvedených po roce 2022~\cite{wang2026visionLanguageFood,fridolfsson2025llmNutrition}, se dělí na specializovaný model trénovaný na vlastním potravinovém datasetu (zastoupený SnapCalorie pro publikovanou metodiku trénování) a~generický multimodální model nasazený přes API (komerční zástupce CalZen)~\cite{coburn2025multimodalNutrition}.
...
Limity představují proměnlivá kvalita uživatelsky přidaných položek vedoucí k~chybám v~nutričních údajích~\cite{li2024evaluating}, nutnost ruční kontroly výsledků foto rozpoznávání a nedostatečné pokrytí lokálních středoevropských produktů.
...
Funkčním rozsahem se neliší od globálního MyFitnessPal, hlavním diferenciátorem je výrazná lokalizace: databáze přizpůsobená regionálnímu trhu s~komunitně rozšiřovanými recepty a značkovými výrobky dostupnými v~ČR a SR, lokalizované rozhraní a foto rozpoznávání zaměřené i na středoevropské pokrmy~\cite{dine4fit2025}.
...
SnapCalorie tak zůstává referenčním příkladem přístupu stavícího primárně na AI rozpoznávání se specializovaným vlastním modelem, jehož hlavním limitem je převážně západní složení trénovacího datasetu.
Riziko zhoršení přesnosti mimo tréninkový region je v~recenzované literatuře doloženo na transformerové architektuře CBiAFormer poklesem z~92,40~\% na ETHZ Food-101 na 72,92~\% na smíšeném ISIA Food-200, což lze nepřímo vztáhnout i na obtížnou přenositelnost SnapCalorie do české a středoevropské kuchyně~\cite{liu2025deepLearning}.

\subsubsection{CalZen}
\label{subsubsec:calzen}

Aplikace CalZen reprezentuje generickou vlnu nástrojů stavících primárně na AI rozpoznávání uvedených v~letech 2024 a~2025, slibujících odhad kalorické a~makroživinové hodnoty z~fotografie pokrmu řádově do několika sekund pro domácí jídla, restaurační pokrmy i~balené potraviny~\cite{calzen2025}.
...
MyFitnessPal nepodporuje češtinu v~uživatelském rozhraní ani ve hlasovém vstupu (omezeno na angličtinu a Spojené státy) a jeho AI asistent dietní omezení využívá pouze v~plánování, nikoli při varování v~okamžiku logování konkrétní položky.
SnapCalorie a CalZen postrádají českou lokalizaci v~celém rozsahu.
Kalorické Tabulky sice nabízejí kompletní českou lokalizaci včetně AI rozpoznávání~\cite{dine4fit2025}, avšak nepodporují hlasový vstup pro logování ani veřejně dokumentované varování při porušení dietních omezení.
Chybějící nezávislá akademická validace komerčních AI funkcí je přitom společným rysem obou skupin~\cite{zecevic2021userPerspectives,liu2025deepLearning}.

Pozici této diplomové práce proto nevymezuje ambice překonat komerční nástroje stavící primárně na AI rozpoznávání v~technické přesnosti, ke které by autorský tým bez zdrojů firem typu SnapCalorie neměl reálné prostředky, nýbrž diferenciace postavená na třech osách: otevřené databázi potravin nad evropským zdrojem Open Food Facts~\cite{openFoodFacts2025}, transparentním zobrazení nejistoty AI výstupu uživateli a~kombinaci česky lokalizovaného hlasového vstupu pro jídla i~fyzické aktivity~\cite{myFitnessPalVoiceLog2025,myFitnessPalLanguages2025} s~proaktivním varováním v~okamžiku AI odhadu jídla, pokud porušuje uživatelem nastavená dietní omezení~\cite{myFitnessPalCoach2025}.
...
Z~uvedeného přehledu vyplývají pro návrh aplikace v~této diplomové práci tři hlavní závěry, které jsou konzistentní i s~výše citovanou analýzou uživatelských stížností~\cite{zecevic2021userPerspectives}.
...
Přístup stavící primárně na AI rozpoznávání je dostupný i~bez vlastního trénovaného modelu skrze multimodální velké jazykové modely nasazené přes API, což otevírá prostor pro rychlou implementaci v~rámci diplomové práce~\cite{wang2026visionLanguageFood}.
Lokální kontext a~podpora české a~středoevropské kuchyně zůstávají strukturálním diferenciátorem vůči konkurenci typu SnapCalorie a~CalZen, primárně orientované na západní a~anglosaský trh~\cite{dine4fit2025,liu2025deepLearning}.
