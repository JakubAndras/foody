...
Postup výkladu odpovídá pořadí, ve kterém byla aplikace stavěna: od volby platformy přes architekturu a~datový model k~integraci modelu umělé inteligence a~uživatelským obrazovkám.
...
Detaily pipeline pro umělou inteligenci, struktura promptů, mechanismus práce s~nejistotou výstupu a~ochrana proti útokům typu \emph{prompt injection} jsou popsány v~sekci~\ref{sec:integraceAi}, omezení vyplývající z~použití vzdáleného modelu v~sekci~\ref{sec:omezeniImplementace}.
...
Transakční entity tvoří dvouúrovňovou hierarchii s~denním záznamem v~kořeni: jídlo je k~dennímu záznamu připojeno vztahem 1:0..N, ingredience je obdobně připojena k~jídlu a~cvičení tvoří paralelní podstrom přímo pod denním záznamem.
...
Obrázek~\ref{fig:screenDashboard} zachycuje obrazovku ve třech typických stavech: ve výchozím prázdném zobrazení bez záznamů, se zobrazenou kartou rozpoznávání jídla v~průběhu volání pipeline umělé inteligence a se zaznamenaným jídlem v~denním přehledu.
...
Obrázek~\ref{fig:screenAskAi} zachycuje obrazovku ve třech typických stavech: výchozí prázdný stav, nápovědu s~ukázkovými otázkami a strukturovanou odpověď doplněnou souhrnem a kalendářem dotčených dní.
...
Obrázek~\ref{fig:screenHealthIntegration} zachycuje vedle této obrazovky také dvě podpůrné obrazovky, které aplikaci doplňují o~uživatelskou nápovědu: kontextovou legendu vysvětlující významy barev kruhových indikátorů v~kalendáři a seznam často kladených dotazů přístupný z~profilu.
