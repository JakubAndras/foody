...
Kvalita rozpoznání je rovněž závislá na kvalitě vstupního obrazu, zejména na osvětlení a úhlu pohledu, což aplikace adresuje pětistránkovou onboardingovou instruktáží, nemůže však zcela kompenzovat nepříznivé pořizovací podmínky.
...
Komunikace s~poskytovatelem modelu umělé inteligence vyžaduje aktivní internetové připojení, takže funkce rozpoznávání jídel z~fotografie, hlasového a textového popisu, analýzy cvičení a dotazů v~přirozeném jazyce nejsou v~režimu offline dostupné, zatímco zbývající funkcionality jsou plně funkční lokálně.
...
Pět funkčních požadavků bylo implementováno pouze částečně.
Mazání jídel a cvičení funguje, mazání účtu chybí.
Čárový kód rozlišuje šest typů chyb, zatímco pipeline pro umělou inteligenci vrací generické selhání.
Vyhledávání v~historii je fulltextové se~zpožděným spouštěním dotazu, ale bez klasického dropdown menu.
Integrace energetického výdeje má pouze přepínače pro výdej a rollover bez koeficientů podle typu aktivity.
Skrytí nebo zobrazení pokročilých funkcí je řešeno individuálními přepínači bez explicitního režimu \emph{basic} versus \emph{advanced}.
